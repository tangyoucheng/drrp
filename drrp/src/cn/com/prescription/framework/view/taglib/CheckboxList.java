...
public class CheckboxList extends Component {
     ...
     * @param _stack スタック
     */
    public CheckboxList(ValueStack _stack) {
        super(_stack);
    }
    ...
    public boolean start(Writer _writer) {
        try {
            ...
            for (CodeValueRecord record_ : dataSource_) {
                ...
                sb_.append("<td><input type=\"checkbox\" name=\"");
                ...
                sb_.append(this.name);
                sb_.append((i + 1));
                sb_.append("\" class=\"checkboxLabel\">");
                sb_.append(BeanUtils.getProperty(record_, this.listValue));
                sb_.append("</label></td>");

                i++;
            }
            sb_.append("<input type=\"hidden\" id=\"__multiselect_");
            sb_.append(this.name);
            sb_.append("\" name=\"__multiselect_");
            sb_.append(this.name);
            sb_.append("\" value=\"\" />");
