 ...
 * @author bpchikazawa
 */
public class DailyRollingAppender extends RollingFileAppender {

    /** 最終書込ファイル */
    private String lastFile = null;

    /** ファイル名パターン */
    private Pattern PATTERN_FILE_NAME = Pattern.compile("^((\\d{8})_)?+(.+?)(\\.\\d+?)??$");
    ...
    private void deleteOldFile(Date _nowDate) {
        try {
            ...
            // ログ保存期間
            int saveRange_ = NumberUtils.toInt(StandardConstantsIF.KYOTU_SYSTEM_RELATED_LOG_SAVE_KIKAN_MONTH_SU);
            ...
            for (File file_ : files_) {
                ...
                // 保存期間を足す
                fileDate_ = DateUtils.addDays(fileDate_, saveRange_);
    ...
    private String getDateFileName(Date _logDate) {
        ...
        // 日付を設定
        String newFile_ = StringUtils.join(new Object[] { dateString_, getOriginFileName(this.fileName) }, '_');

        // ファイル名生成
        return new File(StandardConstantsIF.KYOTU_SYSTEM_RELATED_LOG_SAVE_FOLDER_ROOT_PATH, newFile_).getAbsolutePath();
