...
public class FetchResultSetHandler implements ResultSetHandler {
    ...
    public FetchResultSetHandler(final Class<?> beanClass, final BeanMetaData beanMetaData,
    ...
    protected ResultSetHandler createResultSetHandler(FetchHandler<?> fetchHandler) {
        ...
        if (TypeUtil.isSimpleType(parameterClass)) {
            ...
            // Dto
            final DtoMetaData dtoMetaData = dtoMetaDataFactory.getDtoMetaData(parameterClass);
            resultSetHandler = new FetchDtoMetaDataResultSetHandler(dtoMetaData, createRowCreator(), fetchHandler);
        } else {
            // Bean
            resultSetHandler = new FetchBeanMetaDataResultSetHandler(beanMetaData, createRowCreator(),
