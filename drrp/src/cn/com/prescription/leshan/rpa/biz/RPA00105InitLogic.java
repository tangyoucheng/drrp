...
import cn.com.prescription.leshan.common.data.RpmPrescription01Dao;
...
import cn.com.prescription.leshan.rpa.action.form.RPA0010501Dto;
 ...
 */
public class RPA00105InitLogic extends StandardBiz implements StandardLogic {
     ...
     * 处方DAO
     */
    RpmPrescription01Dao rpmPrescription01Dao = null;

    /**
     * 门店DAO
     */
    RpmStoreDao rpmStoreDao = null;
    ...
    /**
     * RPA00105InitLogic的构造。
     */
    public RPA00105InitLogic() {
        super();
    }
    ...
    public StandardEventResult service(StandardEvent _event) throws ApplicationException, SystemException {
        ...
        if (rpmPatient01Model != null) {
            ...
            // 处方配药者
            // 处方配药者名
            inForm_.setDispenseUserName(rpmPatient01Model.getDispenseUserName());
            // 处方审核者
            // 处方审核者名
            inForm_.setConfirmUserName(UserSessionUtils.getUserSessionInfo().getUserName());
            // 门诊/住院号
        ...
        if (returnList != null && !returnList.isEmpty()) {
            List<RPA0010501Dto> _subForm1 = new ArrayList<RPA0010501Dto>();
            for (RpmPrescriptionDrug01Model rpmPrescriptionDrug01Model : returnList) {
                RPA0010501Dto rpa0010501Dto = new RPA0010501Dto();
                rpa0010501Dto.setDrugId(rpmPrescriptionDrug01Model.getDrugId());
                rpa0010501Dto.setDrugName(rpmPrescriptionDrug01Model.getDrugName());
                rpa0010501Dto.setManufacturerName(rpmPrescriptionDrug01Model.getManufacturerName());
                rpa0010501Dto.setPrice(rpmPrescriptionDrug01Model.getPrice());
                rpa0010501Dto.setUnit(rpmPrescriptionDrug01Model.getUnit());
                rpa0010501Dto.setQuantity(rpmPrescriptionDrug01Model.getQuantity());
                _subForm1.add(rpa0010501Dto);
            }
        ...
        // 设定第一次审核标识
        inForm_.setFirstReviewFlag("");

        // 编辑打印用的处方单编号
        RpmPrescriptionCondition _condition = new RpmPrescriptionCondition();
        _condition.setPrescriptionId(inForm_.getPrescriptionId());
        int prescriptionNumber = rpmPrescription01Dao.selectPrescriptionNumber(_condition);
        String _prescriptionNumber = StringUtils.leftPad(StringUtils.defaultString(prescriptionNumber), 10, "0");
        RpmStoreModel storeModel = rpmStoreDao.select(new RpmStoreCondition());
        if (storeModel != null) {
            _prescriptionNumber = storeModel.getRpCodePrefix() + _prescriptionNumber;
        }
        inForm_.setPrescriptionNumber(_prescriptionNumber);
        // 出力情報設定
     ...
     * 处方DAO的设定
     * 
     * @param _rpmPrescription01Dao 处方DAO
     */
    public void setRpmPrescription01Dao(RpmPrescription01Dao _rpmPrescription01Dao) {
        this.rpmPrescription01Dao = _rpmPrescription01Dao;
    }
