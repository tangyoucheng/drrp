...
public class RPA00203UpdateLogic extends StandardBiz implements StandardLogic {
	...
	private RpmPatientModel patientCheck(RPA00203Form _inForm) throws ApplicationException, SystemException {
		...
		if (returnPatientModel_ != null && !CheckUtils.isEqual(_inForm.getLastUpdateDate(),
			...
			this.addErrorMessage(MessageUtils.getMessage("E00006", "更新", "登录",
					returnPatientModel_.getLastUpdateUserName(), DateUtils.format(
							returnPatientModel_.getLastUpdateDate(), LeshanConstantsIF.DATE_FORMAT_YYYY_MM_DD_HH_MM)));
			this.errorEnd();
		}
	...
	private void updatePatient(RPA00203Form _inForm, RpmPatientModel _patientModel)
		...
		// 民族
		_patientModel.setNation(_inForm.getNation());
		// 出生地
		_patientModel.setPlaceOfBirth(_inForm.getPlaceOfBirth());
		// 婚况
		_patientModel.setMaritalStatus(_inForm.getMaritalStatus());
		// 农历生日
		_patientModel.setLunarBirthday(DateUtils.format(_inForm.getLunarBirthday(),
				LeshanConstantsIF.DATE_FORMAT_YYYY_YEAR_MM_MONTH_DD_DATE, LeshanConstantsIF.DATE_FORMAT_YYYYMMDD));
		// 出生时间
		_patientModel.setTimeOfBirth(DateUtils.format(_inForm.getTimeOfBirth(),
				LeshanConstantsIF.DATE_FORMAT_HH_HOUR_MM_MINUTE, LeshanConstantsIF.DATE_FORMAT_HHMM));
		// 属相
		_patientModel.setZodiac(_inForm.getZodiac());
		// 单位
		_patientModel.setCompany(_inForm.getCompany());
		// 职业
		_patientModel.setProfession(_inForm.getProfession());
