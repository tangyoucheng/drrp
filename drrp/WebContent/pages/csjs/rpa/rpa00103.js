...
$(document).ready(function(){
    ...
    //处方类型切换
    $('input[name="rpa00103Form.prescriptionType"]').click(function() {
    ...
    // 处方状态为审核完成的场合
    if ($('#hiddenPrescriptionStatus').val() == '4') {
        $('#rpa00103Action').find('input[type=text]').attr("class", "readonlydata");
        $('#rpa00103Action').find('input[type=text]').attr("readonly", true);
        
        $('#rpa00103Action').find('textarea').removeClass("mustdata");
        $('#rpa00103Action').find('textarea').attr("class", "readonlydata");
        $('#rpa00103Action').find('textarea').attr("readonly", true);
        //处方类型初始化
        var prescriptionType = $('input[name="rpa00103Form.prescriptionType"]:checked').val();
        if (prescriptionType == "1") {
            $("input[name='rpa00103Form.prescriptionType'][value='1']").attr("disabled",true);
        }
        if (prescriptionType == "2") {
            $("input[name='rpa00103Form.prescriptionType'][value='2']").attr("disabled",true);
        }
    }
});
...
function doWritePrescriptionCallback(_rerurnObject) {
  ...
  //    var i = new Image();
  //    i.src = _rerurnObject;
  //    $(i).appendTo($('#rpa00103ImagePreview'));
...
function doAddDrugCallback(_rerurnObject) {
  if (_rerurnObject != undefined) {
      ...
      drugTableDataHtml += '<input name="rpa00103Form.subForm1['+$('#table_drguListInfo tbody tr').length+'].quantity" type="text" style="width:85px;" class="mustdata" maxLength="10"></input>';
      drugTableDataHtml += '</td>';
      // 删除和隐藏变量
      drugTableDataHtml += '<td style="font-size:1em;text-align: center;">';
      drugTableDataHtml += '<input name="rpa00103Form.subForm1['+$('#table_drguListInfo tbody tr').length+'].drugId" type="hidden" value="'+_rerurnObject.drugId+'" />';
      drugTableDataHtml += '<input name="rpa00103Form.subForm1['+$('#table_drguListInfo tbody tr').length+'].drugName" type="hidden" value="'+_rerurnObject.drugName+'" />';
      drugTableDataHtml += '<input name="rpa00103Form.subForm1['+$('#table_drguListInfo tbody tr').length+'].manufacturerName" type="hidden" value="'+_rerurnObject.manufacturerName+'" />';
      drugTableDataHtml += '<input name="rpa00103Form.subForm1['+$('#table_drguListInfo tbody tr').length+'].price" type="hidden" value="'+_rerurnObject.price+'" />';
